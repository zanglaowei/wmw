...
/*pos*/	
/*alipay*/
	 $alipay['email']=I('alipayemail');	 
	 $alipay['key']=I('alipaykey');	
	 $alipay['partner']=I('alipaypartner');
	 $maps['status']=I('alipayvalue');
	 $maps['status']=I('alipayvalue');
	 $ali['alipay']=$alipay;
     $alidata['name']='alipay';
     $maps['extra']='email:'.$alipay['email'].',key:'.$alipay['key'].',partner:'.$alipay['partner'];
     $maps['value']= json_encode($ali);
	// dump($maps);
...
/*alipay*/
/*qq*/
	 $tenpay['key']=I('tenpaykey');	
	 $tenpay['partner']=I('tenpaypartner');
	 $tenmap['status']=I('tenpayvalue'); 
	 $ten['tenpay']=$tenpay;
     $tendata['name']='tenpay';
     $tenmap['extra']='key:'.$tenpay['key'].',partner:'.$tenpay['partner'];
     $tenmap['value']= json_encode($ten);
	 if ($tenmap['status']) {
...
/*qq*/	 
	 ...
	 $this->success('操作成功');	
	  unset($map); 
	   ...
	   $dataa=$Config->field('name')->where('cate=2')->select();
		...
		$map['status']=I($value['name'].'value');
		 $da[$value['name']]['APP_KEY']=I($value['name'].'APP_KEY');	 
	     $da[$value['name']]['APP_SECRET']=I($value['name'].'APP_SECRET');
		 $maps['name']=$value['name'];
     $map['extra']='APP_KEY:'.$da[$value['name']]['APP_KEY'].',APP_SECRET:'.$da[$value['name']]['APP_SECRET'];
     $map['value']= json_encode($da);
	 if ($map['status']) {
	$Config->where($maps)->save($map); 
	...
	//授权管理
	//本程序基本功能免费，如果购买高级功能请购买授权
    //本程序为开放接口认证，修改此处也无用。基本功能	
